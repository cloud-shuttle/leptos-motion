...
impl PathMorphManager {
    ...
    fn animate_frame(&mut self) -> std::result::Result<(), String> {
        ...
        // Interpolate paths
        self.current_path = Some(Self::interpolate_paths(start_path, end_path, eased_progress));
            ...
            let current = match self.state {
                SharedElementState::Transitioning => {
                    ElementBounds {
                        ...
                        opacity: original.opacity + (shared.opacity - original.opacity) * eased_progress,
                    }
                }
                SharedElementState::Returning => {
                    ElementBounds {
...
mod tests {
    ...
    fn test_shared_element_transition() {
        ...
        let shared_bounds = ElementBounds {
            x: 100.0, y: 100.0, width: 200.0, height: 200.0, opacity: 0.8,
        };
