...
fn test_unused_dependency_detection() {
    ...
    assert!(
        auditor
            .unused_dependencies
            .contains(&"unused-crate".to_string())
    );
}
...
fn test_oversized_dependency_detection() {
    ...
    assert!(
        auditor
            ...
            .contains(&"large-crate".to_string())
    );
}
...
fn test_optimization_recommendations() {
    ...
    assert!(
        recommendations
            .iter()
            .any(|r| r.contains("unused dependencies"))
    );
    assert!(
        recommendations
            .iter()
            .any(|r| r.contains("oversized dependencies"))
    );
}
...
fn test_feature_flag_optimization() {
    ...
    assert!(
        !recommendations
            .iter()
            .any(|r| r.contains("duplicate dependencies"))
    );
}
...
fn test_dependency_audit_report() {
    ...
    for (name, version, size, is_optional) in test_deps {
        let dep = DependencyInfo {
            ...
            is_optional,
            features: vec![],
            used_by: if name == "unused-crate" {
                vec![]
            } else {
                vec!["leptos-motion".to_string()]
            },
        };
        auditor.add_dependency(dep);
    }
