...
impl LayoutInfo {
    ...
    pub fn calculate_transform_to(&self, target: &LayoutInfo) -> HashMap<String, AnimationValue> {
        ...
        if rotation_delta != 0.0 {
            transform.insert("rotateZ".to_string(), AnimationValue::Number(rotation_delta));
        }
        
        // Calculate opacity differences
        let opacity_delta = target.opacity - self.opacity;
        if opacity_delta != 0.0 {
            transform.insert("opacity".to_string(), AnimationValue::Number(target.opacity));
        }
...
impl FLIPManager {
    ...
    fn play_animation(&mut self) -> std::result::Result<(), JsValue> {
        let first_layout = self.first_layout.as_ref().ok_or_else(|| JsValue::from_str("No first layout recorded"))?;
        let last_layout = self.last_layout.as_ref().ok_or_else(|| JsValue::from_str("No last layout recorded"))?;
        let element = self.current_element.as_ref().ok_or_else(|| JsValue::from_str("No element set"))?;
    ...
    fn apply_layout(&self, element: &Element, layout: &LayoutInfo) -> std::result::Result<(), JsValue> {
    ...
    fn invert_element(&self, element: &Element, first: &LayoutInfo, last: &LayoutInfo) -> std::result::Result<(), JsValue> {
        let style = element.style("");
    ...
    fn animate_to_final(&mut self, element: &Element, _transform: &HashMap<String, AnimationValue>) -> std::result::Result<(), JsValue> {
    ...
    fn start_animation(&mut self, element: &Element, target: &HashMap<String, AnimationValue>) -> std::result::Result<(), JsValue> {
        let style = element.style("");
...
mod tests {
    ...
    fn test_layout_info_transform_calculation() {
        // Create mock layouts
        let first_rect = DomRect::new_with_x_and_y_and_width_and_height(0.0, 0.0, 100.0, 100.0).unwrap();
        let last_rect = DomRect::new_with_x_and_y_and_width_and_height(50.0, 50.0, 100.0, 100.0).unwrap();
        ...
        assert_eq!(transform.get("rotateZ"), Some(&AnimationValue::Number(45.0)));
