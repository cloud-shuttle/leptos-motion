...
fn test_wasm_opt_optimization() {
    ...
    for (example, wasm_path) in [
        ...
        if !Path::new(wasm_path).exists() {
            ...
            if !output.status.success() {
                panic!("Failed to build {}: {}", example, String::from_utf8_lossy(&output.stderr));
            }
        }
    }
    
    // Test wasm-opt optimization on showcase
    let original_size = fs::metadata(showcase_wasm).expect("Failed to read original WASM").len();
    ...
    if !output.status.success() {
        panic!("wasm-opt failed: {}", String::from_utf8_lossy(&output.stderr));
    }
    
    let optimized_size = fs::metadata(optimized_path).expect("Failed to read optimized WASM").len();
...
fn test_ultra_minimal_size_target() {
    ...
    if !Path::new(wasm_path).exists() {
        ...
        if !output.status.success() {
            panic!("Failed to build ultra-minimal: {}", String::from_utf8_lossy(&output.stderr));
        }
    }
...
fn test_optimization_analysis() {
    ...
    if !Path::new(wasm_path).exists() {
        ...
        if !output.status.success() {
            panic!("Failed to build showcase: {}", String::from_utf8_lossy(&output.stderr));
        }
    }
    ...
    if !output.status.success() {
        panic!("Failed to run twiggy: {}", String::from_utf8_lossy(&output.stderr));
    }
...
fn test_wasm_opt_levels() {
    ...
    if !Path::new(wasm_path).exists() {
        ...
        if !output.status.success() {
            panic!("Failed to build minimal-showcase: {}", String::from_utf8_lossy(&output.stderr));
        }
    }
    
    let original_size = fs::metadata(wasm_path).expect("Failed to read original WASM").len();
    
    // Test different optimization levels
    let optimization_levels = [
        ("-O1", "target/wasm32-unknown-unknown/release/minimal_o1.wasm"),
        ("-O2", "target/wasm32-unknown-unknown/release/minimal_o2.wasm"),
        ("-O3", "target/wasm32-unknown-unknown/release/minimal_o3.wasm"),
        ("-Oz", "target/wasm32-unknown-unknown/release/minimal_oz.wasm"),
    ];
    ...
    for (level, output_path) in optimization_levels {
        ...
        if !output.status.success() {
            panic!("wasm-opt {} failed: {}", level, String::from_utf8_lossy(&output.stderr));
        }
        
        let optimized_size = fs::metadata(output_path).expect("Failed to read optimized WASM").len();
        ...
        println!("{} optimization: {}KB ({}% reduction)", 
                level, optimized_size / 1024, reduction_percent);
...
fn test_dead_code_elimination() {
    ...
    if !output.status.success() {
        panic!("wasm-opt with DCE failed: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    println!("Dead code elimination: {}KB -> {}KB ({}% reduction)", 
            original_size / 1024, optimized_size / 1024, reduction_percent);
