...
fn test_momentum_threshold_detection() {
    ...
    let high_magnitude: f64 = (high_velocity.0 * high_velocity.0 + high_velocity.1 * high_velocity.1).sqrt();
    let low_magnitude: f64 = (low_velocity.0 * low_velocity.0 + low_velocity.1 * low_velocity.1).sqrt();
...
fn test_momentum_stopping_condition() {
    ...
    let magnitude: f64 = (very_low_velocity.0 * very_low_velocity.0 + very_low_velocity.1 * very_low_velocity.1).sqrt();
