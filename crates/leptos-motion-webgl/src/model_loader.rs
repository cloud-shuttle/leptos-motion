...
impl FromStr for ModelFormat {
    ...
    fn from_str(s: &str) -> Result<Self> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(WebGLError::model_error(&format!("Unsupported model format: {}", s))),
        }
    }
}
...
impl Model {
    ...
    pub fn get_total_vertex_count(&self) -> usize {
        self.meshes.iter().map(|mesh| mesh.geometry.vertices.len()).sum()
    }

    /// Get total index count
    pub fn get_total_index_count(&self) -> usize {
        self.meshes.iter().map(|mesh| mesh.geometry.get_indices_len()).sum()
    }
        ...
        if !faces.is_empty() {
            ...
            if options.scale != 1.0 {
                for vertex in &mut mesh_vertices {
                    ...
                    vertex.set_position([pos[0] * options.scale, pos[1] * options.scale, pos[2] * options.scale]);
                }
            }
            ...
            if options.center {
                ...
                for vertex in &mut mesh_vertices {
                    let pos = vertex.get_position();
                    vertex.set_position([pos[0] - center[0], pos[1] - center[1], pos[2] - center[2]]);
                }
                bounding_box = BoundingBox::empty();
                for vertex in &mesh_vertices {
                    bounding_box.expand(vertex.get_position());
...
impl ModelLoader {
    ...
    pub fn load_from_data(&self, data: &str, format: ModelFormat, options: ModelLoadOptions) -> Result<Model> {
        match format {
            ModelFormat::OBJ => ObjLoader::load(data, options),
            _ => Err(WebGLError::model_error(&format!("Format {:?} not yet implemented", format))),
        }
    }

    /// Load model from URL (async - not implemented yet)
    pub fn load_from_url(&self, _url: &str, _format: ModelFormat, _options: ModelLoadOptions) -> Result<Model> {
        Err(WebGLError::model_error("Async model loading not yet implemented"))
    }
