...
fn test_error_context_creation() {
    ...
    assert_eq!(
        context.additional_info.get("property"),
        Some(&"opacity".to_string())
    );
    assert_eq!(
        context.additional_info.get("value"),
        Some(&"1.0".to_string())
    );
}
...
fn test_complex_animation_error_scenarios() {
    ...
    let perf_error =
        AnimationError::PerformanceBudgetExceeded("Frame budget exceeded: 20ms".to_string());
    let context = ErrorContext::new("animate")
        .with_component("MotionDiv")
    ...
    let memory_error =
        AnimationError::MemoryError("Failed to allocate animation buffer".to_string());
    let context = ErrorContext::new("animate")
        .with_component("MotionDiv")
...
fn test_layout_animation_error_scenarios() {
    ...
    let math_error =
        AnimationError::MathError("Layout calculation failed: division by zero".to_string());
    ...
    let not_impl_error = AnimationError::NotImplemented(
        "Layout animation not supported in this browser".to_string(),
    );
    let context = ErrorContext::new("layout_animate")
        .with_component("LayoutAnimator")
