...
impl CSSOptimizer {
    ...
    pub fn optimize_element(&mut self, element: &Element, properties: &[String]) -> Result<(), JsValue> {
        ...
        if needs_gpu {
            ...
            if properties.iter().any(|prop| prop == "transform") {
                let current_transform = style.get_property_value("transform")
                    .unwrap_or_else(|_| "".to_string());
                
                if !current_transform.contains("translate3d") && !current_transform.contains("translateZ") {
...
impl DOMBatcher {
    ...
    /// Queue a DOM update
    pub fn queue_update(&mut self, element: Element, property: String, value: String, priority: UpdatePriority) {
    ...
    pub fn flush(&mut self) -> Result<usize, JsValue> {
        ...
        self.pending_updates.sort_by(|a, b| b.priority.cmp(&a.priority));
        ...
        for (index, update) in self.pending_updates.iter().enumerate() {
            ...
            // Apply the update
            let style = update.element.unchecked_ref::<web_sys::HtmlElement>().style();
