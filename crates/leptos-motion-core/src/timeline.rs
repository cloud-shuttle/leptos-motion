...
impl Timeline {
    ...
    pub fn add_keyframe_with_easing(&mut self, time: f64, mut keyframe: TimelineKeyframe, easing: Easing) {
    ...
    pub fn scrub_to(&mut self, time: f64) -> Result<()> {
        if !time.is_finite() {
            return Err(AnimationError::InvalidValue("Scrub time must be finite".to_string()));
        }
        
        if time < 0.0 {
            return Err(AnimationError::InvalidValue("Scrub time cannot be negative".to_string()));
        }
        
        if time > self.duration {
            return Err(AnimationError::InvalidValue(
                format!("Scrub time {} exceeds timeline duration {}", time, self.duration)
            ));
        }
    ...
    fn find_surrounding_keyframes(&self, time: f64) -> (Option<&TimelineKeyframe>, Option<&TimelineKeyframe>) {
    ...
    fn get_keyframe_property_value(&self, keyframe: &TimelineKeyframe, property: &str) -> f64 {
        keyframe.properties.get(property).map(|value| {
            match value {
...
impl TimelinePerformanceMetrics {
    ...
    pub fn is_performant(&self) -> bool {
        ...
        (self.keyframe_count < 100 || self.age < Duration::from_secs(10))
    }
...
mod tests {
    ...
    fn test_keyframe_sorting() {
        ...
        timeline.add_keyframe(1.0, TimelineKeyframe::new(1.0).with_property("opacity", AnimationValue::Number(1.0)));
        timeline.add_keyframe(0.0, TimelineKeyframe::new(0.0).with_property("opacity", AnimationValue::Number(0.0)));
        timeline.add_keyframe(0.5, TimelineKeyframe::new(0.5).with_property("opacity", AnimationValue::Number(0.5)));
    ...
    fn test_synchronized_elements() {
        let mut timeline = Timeline::new();
        timeline.add_keyframe(0.0, TimelineKeyframe::new(0.0).with_element("element1".to_string()));
        timeline.add_keyframe(0.5, TimelineKeyframe::new(0.5).with_element("element2".to_string()));
        timeline.add_keyframe(1.0, TimelineKeyframe::new(1.0).with_element("element1".to_string())); // Duplicate
