...
fn test_timeline_sequence_with_transitions() {
    ...
    let step = TimelineStep::new("fade_step".to_string(), target, 1.0)
    ...
    assert_eq!(fade_step.transition.as_ref().unwrap().ease, Easing::EaseInOut);
}
...
fn test_timeline_sequence_performance() {
    ...
    assert_eq!(step.target.get("opacity").unwrap(), &AnimationValue::Number(0.5));
}
...
fn test_timeline_player_complex_sequence() {
    ...
    assert_eq!(current_step.target.get("opacity").unwrap(), &AnimationValue::Number(1.0));
    
    // Update to second step
    player.update(0.7); // 0.1 delay + 0.5 duration + 0.1 buffer
    let current_step = player.current_step().unwrap();
    assert_eq!(current_step.id, "step2");
    assert_eq!(current_step.target.get("rotateZ").unwrap(), &AnimationValue::Number(45.0));
}
