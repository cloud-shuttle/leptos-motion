...
fn test_lazy_loading_on_demand() {
    ...
    loader
        .load_module("gestures", "gesture_module".to_string())
        .unwrap();
    ...
    // Load layout module
    loader
        .load_module("layout", "layout_module".to_string())
        .unwrap();
...
fn test_lazy_loading_no_duplicates() {
    ...
    assert_eq!(
        loader.get_module("gestures"),
        Some("gesture_module".to_string())
    );
}
...
fn test_feature_based_code_splitting() {
    ...
    for feature in &features {
        if feature == &"gestures" || feature == &"layout" {
            loader
                .load_module(feature, format!("{}_module", feature))
                .unwrap();
...
fn test_dynamic_import_simulation() {
    ...
    for action in &user_actions {
        ...
        if action == &"drag" {
            loader
                .load_module("drag_gestures", "drag_module".to_string())
                .unwrap();
            loaded_actions.push("drag");
        } else if action == &"hover" {
            loader
                .load_module("hover_gestures", "hover_module".to_string())
                .unwrap();
...
fn test_bundle_size_reduction() {
    ...
    let total_size: usize = loader.loaded_modules.lock().unwrap().values().sum();
    ...
    let final_size: usize = loader.loaded_modules.lock().unwrap().values().sum();
...
fn test_lazy_loading_performance() {
    ...
    for i in 0..10 {
        loader
            .load_module(&format!("module_{}", i), format!("content_{}", i))
            .unwrap();
    }
    ...
    assert!(
        ...
        "Loading 10 modules took too long: {:?}",
        duration
    );
}
...
fn test_lazy_loading_module_types() {
    ...
    string_loader
        .load_module("string_module", "string_content".to_string())
        .unwrap();
    assert_eq!(
        string_loader.get_module("string_module"),
...
fn test_lazy_loading_cleanup() {
    ...
    // Load some modules
    loader
        .load_module("module1", "content1".to_string())
        .unwrap();
    loader
        .load_module("module2", "content2".to_string())
        .unwrap();
    assert_eq!(loader.loaded_count(), 2);
