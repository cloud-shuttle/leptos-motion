...
fn test_core_bundle_size() {
    ...
    if !Path::new(wasm_path).exists() {
        ...
        let output = Command::new("cargo")
            .args(&["build", "--release", "--target", "wasm32-unknown-unknown", "-p", "leptos-motion-core"])
            .output()
            .expect("Failed to build core crate");
        
        if !output.status.success() {
            panic!("Failed to build core crate: {}", String::from_utf8_lossy(&output.stderr));
        }
    }
...
fn test_minimal_showcase_bundle_size() {
    ...
    if !Path::new(wasm_path).exists() {
        ...
        if !output.status.success() {
            panic!("Failed to build minimal showcase example: {}", String::from_utf8_lossy(&output.stderr));
        }
    }
    ...
    assert!(size_kb <= TARGET_MINIMAL_SIZE_KB, 
        "Minimal showcase bundle size {}KB exceeds target {}KB", size_kb, TARGET_MINIMAL_SIZE_KB);
}
...
fn test_identify_bundle_bloat() {
    ...
    if !output.status.success() {
        panic!("Failed to run twiggy dominators: {}", String::from_utf8_lossy(&output.stderr));
    }
