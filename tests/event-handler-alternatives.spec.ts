    ...
    const manualSignalUpdateWorks = await page.evaluate(() => {
      try {
        ...
        const globalFunctions = Object.keys(window).filter(
          key => typeof (window as any)[key] === 'function'
        );
  ...
  test('Step 5: Try to understand Leptos event handler processing', async ({ page }) => {
    ...
    const leptosEventProcessing = await page.evaluate(() => {
      try {
        ...
        const hasLeptosEventData =
          body.hasAttribute('data-leptos-events') || body.hasAttribute('data-leptos-handlers');

        // Look for any hidden elements or data that might contain event info
        const hiddenElements = document.querySelectorAll('[style*="display: none"], [hidden]');
        const hasHiddenEventData = Array.from(hiddenElements).some(
          el => el.textContent?.includes('event') || el.textContent?.includes('handler')
        );
