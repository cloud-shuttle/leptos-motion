...
fn test_complex_animation_state_management() {
    ...
    set_animation_state.update(|state| {
        ...
        state
            .current_values
            .insert("x".to_string(), AnimationValue::Pixels(25.0));
        state
            .current_values
            .insert("y".to_string(), AnimationValue::Pixels(50.0));
        state
            .current_values
            .insert("opacity".to_string(), AnimationValue::Number(0.75));
    });
    ...
    assert_eq!(
        state.current_values.get("x"),
        Some(&AnimationValue::Pixels(25.0))
    );
    assert_eq!(
        state.current_values.get("y"),
        Some(&AnimationValue::Pixels(50.0))
    );
    assert_eq!(
        state.current_values.get("opacity"),
        Some(&AnimationValue::Number(0.75))
    );
}
...
impl WasmMotionController {
    ...
    pub fn update_animation_value(
        &mut self,
        name: &str,
        value: f32,
    ) -> std::result::Result<(), AnimationError> {
        if let Some((_, write_signal)) = self.signals.get(name) {
            write_signal.set(value);
            Ok(())
        } else {
            Err(AnimationError::InvalidProperty {
                property: format!("Signal '{}' not found", name),
            })
        }
    }
