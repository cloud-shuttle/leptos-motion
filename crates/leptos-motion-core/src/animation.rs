...
impl Keyframes {
    ...
    pub fn interpolate_at(&self, progress: f64) -> Option<AnimationTarget> {
        ...
        for i in 0..self.times.len() - 1 {
            if progress >= self.times[i] && progress <= self.times[i + 1] {
                let segment_progress = (progress - self.times[i]) / (self.times[i + 1] - self.times[i]);
                let eased_progress = self.easings[i].evaluate(segment_progress);
                
                return Some(self.interpolate_targets(&self.values[i], &self.values[i + 1], eased_progress));
            }
        }
    ...
    fn interpolate_targets(&self, from: &AnimationTarget, to: &AnimationTarget, progress: f64) -> AnimationTarget {
        ...
        for property in properties {
            let from_value = from.get(property).cloned().unwrap_or(AnimationValue::Number(0.0));
            let to_value = to.get(property).cloned().unwrap_or(AnimationValue::Number(0.0));
            
            result.insert(property.clone(), from_value.interpolate(&to_value, progress));
        }
...
mod tests {
    ...
    fn test_variants() {
        let variants = Variants::new()
            ...
            .add_variant("visible", animate!("opacity" => AnimationValue::Number(1.0)))
            .initial("hidden")
            .animate("visible");
