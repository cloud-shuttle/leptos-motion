...
fn test_texture_format_conversions() {
    // Test RGB format
    assert_eq!(TextureFormat::RGB.to_webgl_internal_format(), WebGl2RenderingContext::RGB);
    assert_eq!(TextureFormat::RGB.to_webgl_format(), WebGl2RenderingContext::RGB);
    
    // Test RGBA format
    assert_eq!(TextureFormat::RGBA.to_webgl_internal_format(), WebGl2RenderingContext::RGBA);
    assert_eq!(TextureFormat::RGBA.to_webgl_format(), WebGl2RenderingContext::RGBA);
    
    // Test Luminance format
    assert_eq!(TextureFormat::Luminance.to_webgl_internal_format(), WebGl2RenderingContext::LUMINANCE);
    assert_eq!(TextureFormat::Luminance.to_webgl_format(), WebGl2RenderingContext::LUMINANCE);
    
    // Test Alpha format
    assert_eq!(TextureFormat::Alpha.to_webgl_internal_format(), WebGl2RenderingContext::ALPHA);
    assert_eq!(TextureFormat::Alpha.to_webgl_format(), WebGl2RenderingContext::ALPHA);
}

/// Test texture type conversions
#[wasm_bindgen_test]
fn test_texture_type_conversions() {
    assert_eq!(TextureType::UnsignedByte.to_webgl_type(), WebGl2RenderingContext::UNSIGNED_BYTE);
    assert_eq!(TextureType::Float.to_webgl_type(), WebGl2RenderingContext::FLOAT);
    assert_eq!(TextureType::HalfFloat.to_webgl_type(), WebGl2RenderingContext::HALF_FLOAT);
}

/// Test texture wrapping conversions
#[wasm_bindgen_test]
fn test_texture_wrapping_conversions() {
    assert_eq!(TextureWrapping::Repeat.to_webgl_wrapping(), WebGl2RenderingContext::REPEAT);
    assert_eq!(TextureWrapping::ClampToEdge.to_webgl_wrapping(), WebGl2RenderingContext::CLAMP_TO_EDGE);
    assert_eq!(TextureWrapping::MirroredRepeat.to_webgl_wrapping(), WebGl2RenderingContext::MIRRORED_REPEAT);
}

/// Test texture filtering conversions
#[wasm_bindgen_test]
fn test_texture_filtering_conversions() {
    assert_eq!(TextureFiltering::Nearest.to_webgl_filtering(), WebGl2RenderingContext::NEAREST);
    assert_eq!(TextureFiltering::Linear.to_webgl_filtering(), WebGl2RenderingContext::LINEAR);
    assert_eq!(TextureFiltering::LinearMipmapLinear.to_webgl_filtering(), WebGl2RenderingContext::LINEAR_MIPMAP_LINEAR);
}
...
fn test_texture_with_custom_config() {
    ...
    let result = Texture::from_pixels(&context, "custom_texture", pixels, 2, 2, Some(config.clone()));
    assert!(result.is_ok());
...
fn test_texture_format_channels() {
    ...
    let texture = Texture::from_pixels(&context, "luminance_test", pixels, 2, 1, Some(config)).unwrap();
...
fn test_texture_manager_clear() {
    ...
    manager.create_from_pixels("texture1", pixels1, 1, 1, None).unwrap();
    manager.create_from_pixels("texture2", pixels2, 1, 1, None).unwrap();
