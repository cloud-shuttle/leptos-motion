...
test.describe('3D Transform Demo', () => {
  ...
  test('should display 3D transform controls', async ({ page }) => {
    await expect(page.locator('.transform-3d-demo__controls h3')).toContainText(
  ...
  test('should handle slider interactions', async ({ page }) => {
    const translationXSlider = page.locator(
      '.control-group:has-text("Translation X:") input[type="range"]'
    );
