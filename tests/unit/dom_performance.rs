...
fn test_css_optimizer_memory_usage() {
    ...
    let transform_memory =
        optimizer.transform_cache.len() * std::mem::size_of::<(String, String)>();
...
fn test_css_optimizer_clone() {
    ...
    assert_eq!(
        cloned.get_cached_transform(&"key1".to_string()),
        Some(&"value1".to_string())
    );
    assert_eq!(
        cloned.get_cached_opacity(&"key2".to_string()),
        Some(&"0.5".to_string())
    );
}
