...
test('interactive demo test - click buttons and verify animations', async ({ page }) => {
  ...
  try {
    ...
    if (buttonCount > 0) {
      ...
      const afterClickLogs = logs.filter(
        log => log.includes('Animation triggered') || log.includes('Returning')
      );
      afterClickLogs.forEach(log => console.log(log));

      // Check if we got the expected active animation log
      const hasActiveAnimation = afterClickLogs.some(
        log => log.includes('is_active: true') || log.includes('active animation')
      );
      ...
      const afterSecondClickLogs = logs.filter(
        log => log.includes('Animation triggered') || log.includes('Returning')
      );
      afterSecondClickLogs.forEach(log => console.log(log));

      // Verify we have multiple animation state changes
      const totalAnimationLogs = logs.filter(log => log.includes('Animation triggered'));
      ...
      const hasInitialAnimation = logs.some(log => log.includes('Animation triggered'));
