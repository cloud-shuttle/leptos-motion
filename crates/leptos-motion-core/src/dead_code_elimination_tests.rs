...
impl DeadCodeEliminationAnalyzer {
    ...
    pub fn new() -> Self {
        ...
        let total_savings_bytes: u64 = dead_code_modules
            .values()
    ...
    pub fn get_feature_flag(&self, module_name: &str) -> Option<&String> {
        self.dead_code_modules
            .get(module_name)
            .map(|m| &m.feature_flag)
    }
...
mod tests {
    ...
    fn test_feature_flag_mapping() {
        ...
        assert_eq!(
            analyzer.get_feature_flag("developer_tools"),
            Some(&"developer-tools".to_string())
        );
        assert_eq!(
            analyzer.get_feature_flag("advanced_examples"),
            Some(&"advanced-examples".to_string())
        );
        assert_eq!(
            analyzer.get_feature_flag("ecosystem_integration"),
            Some(&"ecosystem-integration".to_string())
        );
    ...
    fn test_bundle_size_calculation() {
        ...
        let full_size = analyzer.calculate_optimized_size(
            base_size,
            &[
                "developer-tools",
                "advanced-examples",
    ...
    fn test_production_build_size_target() {
        ...
        let reduction_percentage =
            ((base_size - production_size) as f64 / base_size as f64) * 100.0;
