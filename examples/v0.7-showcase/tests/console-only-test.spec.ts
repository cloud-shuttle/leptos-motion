...
test('verify reactive system is working via console logs only', async ({ page }) => {
  ...
  const hasAnimationTriggered = animationLogs.some(log => 
    log.includes('Animation triggered')
  );
  
  const hasReturningAnimation = animationLogs.some(log => 
    log.includes('Returning')
  );
  
  const hasIdleAnimation = animationLogs.some(log => 
    log.includes('idle animation')
  );
