...
impl E2ETestEnvironment {
    pub fn new(config: E2EConfig) -> Self {
        ...
        let performance_monitor = if config.performance_monitoring.monitor_fps || config.performance_monitoring.monitor_memory {
    ...
    fn setup_viewport(&self) -> Result<(), JsValue> {
        ...
        viewport_meta.set_attribute("content", &format!(
...
impl<'a> E2EAssertions<'a> {
    ...
    pub fn assert_element_visible(&self, element: &Element) -> Result<(), String> {
        let style = self.env.window.get_computed_style(element)
        ...
        let visibility = style.get_property_value("visibility")
            .unwrap_or_default();
        let opacity = style.get_property_value("opacity")
            .unwrap_or_default();
    ...
    pub fn assert_animation_completed(&self, element: &Element, property: &str, expected_value: &str) -> Result<(), String> {
        let start_time = js_sys::Date::now();
    ...
    pub fn assert_performance_threshold(&self, metrics: &PerformanceMetrics) -> Result<(), String> {
        ...
        if metrics.total_animation_time_ms > self.env.config.performance_monitoring.performance_threshold_ms {
            return Err(format!(
                "Animation time too long: {}ms (expected <= {}ms)",
                metrics.total_animation_time_ms,
                self.env.config.performance_monitoring.performance_threshold_ms
            ));
        }
    ...
    fn get_computed_style_property(&self, element: &Element, property: &str) -> Result<String, String> {
        let computed_style = self.env.window.get_computed_style(element)
            .map_err(|_| "Failed to get computed style")?
            .ok_or("No computed style found")?;
        
        Ok(computed_style.get_property_value(property).unwrap_or_default())
    }
}
...
impl E2ETestSuite {
        ...
        F: Fn(&E2ETestEnvironment) -> Result<(), String> + 'static,
    {
        ...
        let test = Box::new(move |env: &mut E2ETestEnvironment| {
            env.run_test(&test_name, &test_fn)
        });
    ...
    pub fn run_all_tests(&self) -> Vec<TestResult> {
        ...
        if let Err(e) = env.cleanup() {
            web_sys::console::error_1(&format!("Failed to cleanup test environment: {:?}", e).into());
        }
