...
impl VisualTestHelper {
    ...
    fn create_test_element(&self, id: &str, class: &str) -> web_sys::Element {
        ...
        element.set_attribute("style", "width: 100px; height: 100px; background: red; position: absolute;").unwrap();
        element
    }
    ...
    fn get_opacity(&self, element: &web_sys::Element) -> f64 {
        self.get_computed_style(element, "opacity").parse().unwrap_or(1.0)
    }
    
    fn get_position(&self, element: &web_sys::Element) -> (f64, f64) {
        let left = self.get_computed_style(element, "left").replace("px", "").parse().unwrap_or(0.0);
        let top = self.get_computed_style(element, "top").replace("px", "").parse().unwrap_or(0.0);
        (left, top)
    }
}
...
async fn test_fade_in_animation_visual_consistency() {
    ...
    assert!((final_opacity - 1.0).abs() < 0.01, "Opacity should be 1.0, got {}", final_opacity);
}
...
async fn test_slide_animation_visual_consistency() {
    ...
    assert!((final_x - (initial_x + 200.0)).abs() < 1.0, "X position should be {} + 200, got {}", initial_x, final_x);
...
async fn test_scale_animation_visual_consistency() {
    ...
    assert!(final_transform.contains("scale(2)"), "Transform should contain scale(2), got {}", final_transform);
}
...
async fn test_rotation_animation_visual_consistency() {
    ...
    // Final transform
    let final_transform = helper.get_transform_matrix(&element);
    assert!(final_transform.contains("rotate(360deg)"), "Transform should contain rotate(360deg), got {}", final_transform);
}
...
async fn test_complex_animation_visual_consistency() {
    ...
    assert!((final_opacity - 1.0).abs() < 0.01, "Opacity should be 1.0, got {}", final_opacity);
    assert!(final_transform.contains("scale(1.5)"), "Transform should contain scale(1.5), got {}", final_transform);
}
...
async fn test_spring_animation_visual_consistency() {
    ...
    let (final_x, _) = helper.get_position(&element);
    assert!((final_x - 100.0).abs() < 5.0, "Spring should settle near 100px, got {}", final_x);
}
...
async fn test_animation_interruption_visual_consistency() {
    ...
    let (final_x, _) = helper.get_position(&element);
    assert!((final_x - 0.0).abs() < 1.0, "Should end at 0px, got {}", final_x);
}
...
async fn test_animation_presence_visual_consistency() {
    ...
    assert!((enter_opacity - 1.0).abs() < 0.01, "Enter opacity should be 1.0, got {}", enter_opacity);
    ...
    assert!((exit_opacity - 0.0).abs() < 0.01, "Exit opacity should be 0.0, got {}", exit_opacity);
}
...
async fn test_animation_variants_visual_consistency() {
    ...
    assert!((final_opacity - 1.0).abs() < 0.01, "Opacity should be 1.0, got {}", final_opacity);
    assert!((final_x - 0.0).abs() < 1.0, "X should be 0px, got {}", final_x);
}
...
async fn test_animation_stagger_visual_consistency() {
    ...
    let elements: Vec<web_sys::Element> = (0..3)
        .map(|i| {
            ...
            helper.document.body().unwrap().append_child(&element).unwrap();
            element
        })
        .collect();
    ...
    for element in &elements {
        ...
        assert!((final_opacity - 1.0).abs() < 0.01, "Opacity should be 1.0, got {}", final_opacity);
        assert!((final_y - 0.0).abs() < 1.0, "Y should be 0px, got {}", final_y);
    }
}
