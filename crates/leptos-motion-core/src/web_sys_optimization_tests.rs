...
fn test_core_dom_functionality_after_web_sys_optimization() {
    ...
    let html_element = element.dyn_into::<web_sys::HtmlElement>().expect("Should be HtmlElement");
    let style = html_element.style();
    style.set_property("opacity", "0.5").expect("Should set style");
...
fn test_animation_apis_after_web_sys_optimization() {
    ...
    let html_element = element.dyn_into::<web_sys::HtmlElement>().expect("Should be HtmlElement");
    let style = html_element.style();
    style.set_property("transform", "translateX(100px)").expect("Should set transform");
    assert!(style.get_property_value("transform").unwrap().contains("translateX"));
}
...
fn test_minimal_web_sys_features_sufficient() {
    ...
    style.set_property("opacity", "1").expect("Should set opacity");
    style.set_property("transform", "translateX(0px)").expect("Should set transform");
    style.set_property("transition", "all 0.3s ease").expect("Should set transition");
    
    // Verify all properties were set
    assert_eq!(style.get_property_value("opacity").unwrap(), "1");
    assert!(style.get_property_value("transform").unwrap().contains("translateX"));
    assert!(style.get_property_value("transition").unwrap().contains("0.3s"));
}
