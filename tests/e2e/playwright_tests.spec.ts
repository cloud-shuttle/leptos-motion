...
test.describe('Leptos Motion E2E Tests', () => {
  ...
  test('should animate elements on click', async ({ page }) => {
    ...
    // Get initial scale
    const initialScale = await clickElement.evaluate(el => window.getComputedStyle(el).transform);
    ...
    const finalScale = await clickElement.evaluate(el => window.getComputedStyle(el).transform);
