...
impl E2ETestHelper {
    ...
    fn create_test_app(&self) -> web_sys::Element {
        ...
        app.set_id("test-app");
        app.set_attribute("style", "width: 800px; height: 600px; position: relative;")
            .unwrap();
    ...
    fn assert_style_contains(&self, element: &web_sys::Element, property: &str, expected: &str) {
        ...
        assert!(
            ...
            "Style property '{}' should contain '{}', got '{}'",
            property,
            expected,
            actual
        );
...
async fn test_draggable_element_e2e() {
    ...
    draggable
        .set_attribute(
            "style",
            "width: 100px; height: 100px; background: green; position: absolute; cursor: grab;",
        )
        .unwrap();
    ...
    assert_ne!(
        drag_transform, initial_transform,
        "Element should have moved during drag"
    );
...
async fn test_list_animation_e2e() {
    ...
    let items: Vec<web_sys::Element> = (0..3)
        .map(|i| {
            ...
            item.set_attribute(
                "style",
                "padding: 10px; margin: 5px; background: lightblue;",
            )
            .unwrap();
    ...
    for item in &items {
        ...
        helper.assert_style_contains(
            &helper.get_computed_style(item, "transform"),
            "translateX(0px)",
        );
    }
}
...
async fn test_form_validation_animation_e2e() {
    ...
    input
        .set_attribute("style", "padding: 10px; border: 2px solid #ccc;")
        .unwrap();
    ...
    error
        .set_attribute("style", "color: red; font-size: 12px; display: none;")
        .unwrap();
...
async fn test_page_transition_e2e() {
    ...
    for i in 0..3 {
        let nav_button = helper
            .document
            .get_element_by_id(&format!("nav-{}", i))
            .unwrap();
        ...
        helper.assert_style_contains(
            &helper.get_computed_style(&page, "transform"),
            "translateX(0px)",
        );
    }
}
...
async fn test_performance_under_load_e2e() {
    ...
    for element in &elements {
        ...
        assert!(
            transform.contains("scale(1)")
                || transform.contains("scale(1.2)")
                || transform.contains("scale(0.9)")
        );
    }
}
