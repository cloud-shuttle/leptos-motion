...
fn SpringPhysicsDemo() -> impl IntoView {
    ...
    let toggle = move |_| {
        ...
        web_sys::console::log_1(&format!("Animation triggered, is_active: {}", is_active.get()).into());
    };
    
    let animation_target = signal_animate(move || {
        let is_active_val = is_active.get();
        web_sys::console::log_1(&format!("Returning {} animation", if is_active_val { "active" } else { "idle" }).into());
        
        if is_active_val {
            HashMap::from([
                ("transform".to_string(), "translateX(100px) scale(1.2) rotate(10deg)".to_string()),
                ("background-color".to_string(), "#3b82f6".to_string()),
            ])
        } else {
            HashMap::from([
                ("transform".to_string(), "translateX(0px) scale(1.0) rotate(0deg)".to_string()),
