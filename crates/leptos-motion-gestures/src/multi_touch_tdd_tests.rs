...
fn test_gesture_type_detection_based_on_touch_count() {
    ...
    for (touch_count, expected_type) in test_cases {
        ...
        let touches: Vec<TouchPoint> = (0..touch_count)
            .map(|i| touch_point_fixture(i as u64, 100.0 + (i as f64 * 10.0), 100.0 + (i as f64 * 10.0)))
            .collect();
...
fn test_multi_touch_thread_safety() {
    ...
    for i in 0..5 {
        ...
        let handle = thread::spawn(move || {
            let touches = vec![
                touch_point_fixture(i as u64, 100.0 + (i as f64), 100.0 + (i as f64)),
            ];
...
fn test_gesture_sensitivity() {
    let mut high_sensitivity_detector = MultiTouchGestureDetector::new(
        GestureConfig {
            sensitivity: 0.9,
            ..Default::default()
        }
    ...
    let mut low_sensitivity_detector = MultiTouchGestureDetector::new(
        GestureConfig {
            sensitivity: 0.1,
            ..Default::default()
        }
