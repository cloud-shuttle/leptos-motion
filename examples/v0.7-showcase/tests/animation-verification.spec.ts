...
test('verify animations are working', async ({ page }) => {
  ...
  const animationLogs = logs.filter(
    log =>
      log.includes('Animation triggered') || log.includes('Returning') || log.includes('is_active')
  );
  ...
  const hasInitialAnimation = animationLogs.some(
    log =>
  ...
  try {
    ...
    if (buttons.length > 0) {
      ...
      const hasActiveAnimation = newAnimationLogs.some(
        log =>
