use leptos_motion_core::{
    AnimationError, ErrorContext, RecoveryStrategy, ErrorHandler, DefaultErrorHandler,
    AnimationHandle
};
...
fn test_error_handler_invalid_property() {
    let handler = DefaultErrorHandler::default();
    let error = AnimationError::InvalidProperty { property: "invalid_prop".to_string() };
    let context = ErrorContext::new("test");
...
fn test_error_handler_already_running() {
    let handler = DefaultErrorHandler::default();
    let error = AnimationError::AlreadyRunning { handle: AnimationHandle(1) };
    let context = ErrorContext::new("test");
...
fn test_error_handler_not_found() {
    let handler = DefaultErrorHandler::default();
    let error = AnimationError::NotFound { handle: AnimationHandle(1) };
    let context = ErrorContext::new("test");
...
fn test_error_context_additional_info_mutability() {
    ...
    context.additional_info.insert("new_key".to_string(), "new_value".to_string());
    
    assert_eq!(context.additional_info.get("new_key"), Some(&"new_value".to_string()));
}
...
fn test_error_handler_with_disabled_user_messages() {
    ...
    let error = AnimationError::InvalidProperty { property: "test".to_string() };
