...
impl CSSAnimationManager {
    ...
    pub fn start_animation(&mut self, element_id: &str, animation_type: &str) -> Result<(), String> {
        let class_name = format!("{}-{}", self.config.base_class, animation_type);
        
        if self.active_animations.contains_key(element_id) {
            return Err(format!("Animation already active for element {}", element_id));
        }
    ...
    pub fn stop_animation(&mut self, element_id: &str) -> Result<(), String> {
        if let Some(state) = self.active_animations.get_mut(element_id) {
            ...
            Err(format!("No active animation found for element {}", element_id))
        }
    }
...
fn test_start_animation() {
    ...
    assert_eq!(manager.get_element_class("element1"), Some("animate-fade-in".to_string()));
...
fn test_multiple_animations() {
    ...
    manager.start_animation("element1", "fade-in").unwrap();
    manager.start_animation("element2", "slide-in-left").unwrap();
    ...
    assert_eq!(manager.get_element_class("element1"), Some("animate-fade-in".to_string()));
    assert_eq!(manager.get_element_class("element2"), Some("animate-slide-in-left".to_string()));
    assert_eq!(manager.get_element_class("element3"), Some("animate-scale-in".to_string()));
}
...
fn test_tailwind_integration() {
    ...
    manager.start_animation("element1", "fade-in").unwrap();
    assert_eq!(manager.get_element_class("element1"), Some("transition-fade-in".to_string()));
}
...
fn test_performance_many_animations() {
    ...
    for i in 0..100 {
        ...
        let animation_type = if i % 2 == 0 { "fade-in" } else { "slide-in-left" };
        manager.start_animation(&element_id, animation_type).unwrap();
    }
