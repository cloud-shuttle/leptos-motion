...
fn test_animate_presence_custom_exit_animation() {
    ...
    exit_animation.target.insert("rotateZ".to_string(), AnimationValue::Number(180.0));
    exit_animation.transition.duration = Some(0.5);
    
    let mut presence = AnimatePresence::new(PresenceMode::Wait)
        .with_exit_animation(exit_animation.clone());
...
fn test_animate_presence_per_child_exit_animation() {
    ...
    fade_animation.target.insert("opacity".to_string(), AnimationValue::Number(0.0));
    
    let mut slide_animation = ExitAnimation::default();
    slide_animation.target.insert("y".to_string(), AnimationValue::Number(-100.0));
...
fn test_animate_presence_motion_div_integration() {
    ...
    let mut exit_animation = ExitAnimation::default();
    exit_animation.target.insert("opacity".to_string(), AnimationValue::Number(0.0));
    exit_animation.target.insert("scale".to_string(), AnimationValue::Number(0.5));
    exit_animation.target.insert("rotateZ".to_string(), AnimationValue::Number(180.0));
