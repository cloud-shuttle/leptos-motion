...
fn BasicAnimationsDemo() -> impl IntoView {
    ...
    let create_animation_target = |visible: bool, mode: i32| -> HashMap<String, AnimationValue> {
        ...
        match mode {
            ...
            1 => {
                // Rotate & Slide
                target.insert("rotate".to_string(), AnimationValue::Number(if visible { 0.0 } else { 180.0 }));
                target.insert("x".to_string(), AnimationValue::Pixels(if visible { 0.0 } else { 100.0 }));
            }
            _ => {
                // Color & Border
                target.insert("background".to_string(), AnimationValue::Color(if visible {
                    "linear-gradient(135deg, #667eea 0%, #764ba2 100%)".to_string()
                } else {
                    "linear-gradient(135deg, #f093fb 0%, #f5576c 100%)".to_string()
                }));
                target.insert("border-radius".to_string(), AnimationValue::Pixels(if visible { 10.0 } else { 50.0 }));
            }
        }
    ...
    view! {
        <div class="showcase-card">
            ...
            <div class="demo-area">
                <MotionDiv
                    class="w-20 h-20 mx-auto".to_string()
...
fn GestureInteractionsDemo() -> impl IntoView {
    ...
    let hover_target = move || {
        ...
        if hover_state.get() {
            ...
            target.insert("box-shadow".to_string(), AnimationValue::String("0 20px 40px rgba(0,0,0,0.3)".to_string()));
        } else {
            target.insert("scale".to_string(), AnimationValue::Number(1.0));
            target.insert("rotate".to_string(), AnimationValue::Number(0.0));
            target.insert("box-shadow".to_string(), AnimationValue::String("0 5px 15px rgba(0,0,0,0.1)".to_string()));
        }
        target
    };
    ...
    view! {
        <div class="showcase-card">
            ...
            <div class="demo-area">
                <MotionDiv
                    class="w-24 h-24 mx-auto cursor-pointer".to_string()
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 10px; min-height: 120px;">
                    <For
                        ...
                        children=move |item| {
                            view! {
                                <MotionDiv
                                    class="w-12 h-12 flex items-center justify-center text-white font-bold".to_string()
...
fn KeyframeAnimationsDemo() -> impl IntoView {
    ...
    view! {
        <div class="showcase-card">
            ...
            <div class="demo-area">
                <MotionDiv
                    class="w-16 h-16 mx-auto".to_string()
    ...
    view! {
        <div class="showcase-card">
            ...
            <div class="demo-area" style="position: relative; height: 150px; border: 2px solid #ddd;">
                <MotionDiv
                    class="w-16 h-16 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 cursor-grab".to_string()
...
fn PerformanceDemo() -> impl IntoView {
    ...
    let create_performance_animation = |index: usize, running: bool| -> HashMap<String, AnimationValue> {
        let mut target = HashMap::new();
        if running {
        ...
        <div class="showcase-card">
            ...
            <div class="demo-area" style="position: relative; height: 120px;">
                {(0..animation_count.get()).map(|i| {
                    view! {
                        <MotionDiv
                            class="w-4 h-4 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2".to_string()
...
fn AdvancedFeaturesDemo() -> impl IntoView {
    ...
    let create_advanced_animation = |mode: i32| -> HashMap<String, AnimationValue> {
        ...
        match mode {
            ...
            1 => {
                // Color transitions
                target.insert("background".to_string(), AnimationValue::Color("linear-gradient(135deg, #ff9a9e 0%, #fecfef 100%)".to_string()));
