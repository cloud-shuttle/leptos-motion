...
fn test_multiple_motion_divs_independent() {
    ...
    // Initial state - both divs should have their initial values
    // RED: This will fail because the components don't properly track signals
    let initial_opacity = get_computed_style("opacity");
    assert_eq!(initial_opacity, "0"); // First div should be invisible

    // Change div1 state - should trigger animation
    set_div1_state.set(1);
    let updated_opacity = get_computed_style("opacity");
    assert_eq!(updated_opacity, "1"); // Should become visible

    // Change div2 state - should trigger animation
    set_div2_state.set(1);
    let final_opacity = get_computed_style("opacity");
    assert_eq!(final_opacity, "0"); // Second div should be invisible
}
